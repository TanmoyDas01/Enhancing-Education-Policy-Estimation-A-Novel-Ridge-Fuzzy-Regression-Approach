---
title: 'Fuzzy Alpha-cut Linear Regression: MODIFIED TANAKA DATA'
date: "2018/ 11/ 15"
...
for (a in 1:len.A) {
  # estimated fuzzy regression coefficients using glmnet.
  l_fit <- glmnet(X, l_Y[, a], alpha = 0, lambda = 0)
  r_fit <- glmnet(X, r_Y[, a], alpha = 0, lambda = 0)
  ...
  #l_summary <- summary(lm (l_Y[, a] ~ X))
  #r_summary <- summary(lm (r_Y[, a] ~ X))
  
  #l_Ak[, a] <- l_summary$coefficients[, 1] # Linear regression on X
  #r_Ak[, a] <- r_summary$coefficients[, 1]
  
  #l_Ak_var[, a] <- l_summary$coefficients[, 2]
  #r_Ak_var[, a] <- r_summary$coefficients[, 2]
  
  #betas_L[2:(p+1), a] <- lm (cY_L[, a] ~ sX)$coef[-1] # Linear regression on standardized X
  #betas_L[2:(p+1), a] <- betas_L[2:(p+1), a] / scaleX
  #betas_L[1,a] <- mean(Y_L[, a]) - betas_L[2:(p+1), a] %*% colMeans(X)  # Intercept
  
  #betas_R[2:(p+1), a] <- lm (cY_R[, a] ~ sX)$coef[-1]
  #betas_R[2:(p+1), a] <- betas_R[2:(p+1), a] / scaleX
  #betas_R[1,a] <- mean(Y_R[, a]) - betas_R[2:(p+1), a] %*% colMeans(X)  # Intercept
}
...
rownames(l_Ak_var) <- c("l_A1", "l_A2", "l_A3", "l_A4")

colnames(r_Ak_var) <- c("alpha = 1", "alpha = 0.75", "alpha = 0.5", "alpha = 0.25", "alpha = 0")
rownames(r_Ak_var) <- c("r_A1", "r_A2", "r_A3", "r_A4")
```
...
```{r, echo = FALSE}
l_summary <- cbind(t(l_Ak)[, 2], t(l_Ak_var)[, 1],
                   t(l_Ak)[, 3], t(l_Ak_var)[, 2],
                   t(l_Ak)[, 4], t(l_Ak_var)[, 3])
colnames(l_summary) <- c("l_A1 Estimate", "l_A1 Std.Error",
                         "l_A2 Estimate", "l_A2 Std.Error",
                         "l_A3 Estimate", "l_A3 Std.Error")

r_summary <- cbind(t(r_Ak)[, 2], t(r_Ak_var)[, 1],
                   t(r_Ak)[, 3], t(r_Ak_var)[, 2],
                   t(r_Ak)[, 4], t(r_Ak_var)[, 3])
colnames(r_summary) <- c("r_A1 Estimate", "r_A1 Std.Error",
...
To be sure, compare with the results from `lm`. 
...
To be sure, compare the mid-values of the fuzzy predicted values with the predicted values from `lm`. 
...
Since for the fuzzy linear regression the RMSE and MAPE for the right slope is the same, we choose the left slope, then perform symmetrization. 
...
for (i in 1:(p + 1)) {
  left.slope <- function(x) {l_slopes[i,1] * (x - l_slopes[i,2]) + 1}
  right.slope <- function(x) {l_slopes[i,3] * (x - l_slopes[i,2]) + 1}
  
  min_x <- r_f_Ak[i, 1] 
  max_x <- r_f_Ak[i, 3] 
  
  x_left <- c(min_x, l_f_Ak[i, 2])
  x_right <- c(l_f_Ak[i, 2], max_x)
  x <- c(x_left, x_right)
  ...
  abline(v = l_slopes[i,2], col = "dodgerblue4")
}
...
```{r}
f_1  <- cbind(rep(1, n) * r_f_Ak[1, 1], rep(1, n) * r_f_Ak[1, 2], rep(1, n) * l_f_Ak[1, 3])
f_XL <- X %*% l_f_Ak[2:(p+1), 1]
f_XC <- X %*% l_f_Ak[2:(p+1), 2]
f_XR <- X %*% l_f_Ak[2:(p+1), 3]
...
for(g in unique(y_df$grp)){
  ...
  q = q + geom_polygon(data = dat1, aes(x, y, alpha=0.6), colour="grey20", fill=NA) + 
    geom_polygon(data = dat2, aes(x, y, alpha=0.6), colour="dodgerblue4", fill=NA) +
    ylim(0, 3000) +
    theme_bw() + 
